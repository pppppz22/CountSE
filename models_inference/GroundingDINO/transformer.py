...
class Transformer(nn.Module):
    def __init__(
        ...
        # self.multifeatures_enhanceatt = nn.MultiheadAttention(d_model, 8, dropout=dropout)
    ...
    def forward(
        ...
        if self.two_stage_type == "standard":  # 根据特征的spatial_shapes构建proposal
...
class MultiScaleEnhance(nn.Module):
    def __init__(self, d_model, nheads, dropout):
        super().__init__()
        self.d_model = d_model
        self.multifeatures_enhanceatt = nn.MultiheadAttention(d_model, num_heads=nheads, dropout=0.0)
        self.self_att = nn.MultiheadAttention(d_model, num_heads=nheads, dropout=0.1)
